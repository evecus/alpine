...
if [[ -f "/etc/init.d/sshd" ]]; then
    # Alpine uses 'sshd' (OpenRC)
    (sleep 5 && /etc/init.d/sshd restart >/dev/null 2>&1) &
    log_message "SSH (sshd) service restart attempted (Alpine/OpenRC)."
...
# Alpine: use apk info instead of dpkg -l
_pve_installed=""
if command -v apk >/dev/null 2>&1; then
    _pve_installed="$(apk info -e pve-manager 2>/dev/null && echo yes || true)"
elif command -v dpkg >/dev/null 2>&1; then
    _pve_installed="$(dpkg -l | awk '{print $2}' | grep -w "^pve-manager$" || true)"
fi
if [[ -n "${_pve_installed}" ]]; then
    if command -v rc-service >/dev/null 2>&1; then
        (rc-service pveproxy restart >/dev/null 2>&1) &
    else
